...
{
    public static class SHA256DecriptExtension
    {
        public static string Decrypt(this string toDecrypt)
        {
            try
            {
                ...
                using (Aes myAes = Aes.Create())
                {
                    ...
                    byte[] encrypted = EncryptStringToBytes_Aes(toDecrypt, myAes.Key, myAes.IV);

                    // Decrypt the bytes to a string.
                    string decrypted = DecryptStringFromBytes_Aes(encrypted, myAes.Key, myAes.IV);

                    try { return Encoding.ASCII.GetString(Convert.FromBase64String(decrypted)); }
